...
async function runSimulation() {
    ...
    try {
        ...
        if (viewer3D) {
            viewer3D.setSimulationData(lastSimulationData);
            if (blanketEnabled) {
                viewer3D.updateBlanket(req.blanket_length, req.blanket_thickness);
            }
        }
...
async function runOptimization() {
    ...
    try {
        ...
        optBox.innerHTML = `
            ...
            <button class="btn btn-info" style="margin-top:10px" onclick="applyOptimizedBlanket(${opt.blanket_length}, ${opt.blanket_thickness})">
                应用到仿真
            </button>
        `;
...
function applyOptimizedBlanket(length, thickness) {
    ...
    if (viewer3D) {
        viewer3D.updateBlanket(length, thickness);
    }
    ...
    try {
        ...
        tbody.innerHTML = sims.map(s => `
            <tr>
                ...
                <td>
                    <button class="btn btn-info" onclick="loadSimulationToViewer(${s.id})">查看</button>
                </td>
            </tr>
        `).join('');
    } catch (e) {
            ...
            <div class="alarm-item ${levelCls} ${handledCls}">
                ...
                ${!a.is_handled ? `
                    <div class="alarm-actions">
                        <button class="btn btn-info" onclick="acknowledgeAlarm(${a.id})" style="padding:5px 10px;font-size:11px">确认处理</button>
                        <button class="btn" style="padding:5px 10px;font-size:11px;background:#34495e;color:white" onclick="loadSensorDetail('${a.sensor_id}')">查看详情</button>
                    </div>
...
async function main() {
    try {
        ...
        viewer3D = new Dam3DViewer('threeCanvasContainer');
        ...
        if (viewer3D && sensorConfigs.length > 0) {
            const dataMap = {};
            for (const [id, data] of Object.entries(latestSensorData)) {
                dataMap[id] = data.sensor_value;
            }
            viewer3D.updateSensorConfigs(sensorConfigs, dataMap);
        }
        ...
        setInterval(loadSensorsTable, 30000);
        setInterval(loadSimulationsTable, 60000);

        setTimeout(runDefaultSimulation, 2000);
        ...
        showNotification('系统初始化出错: ' + e.message, 'danger');
    }
}
